...
Button {
    ...
    function _containerColor() {
        ...
        case "elevated": return GreeterTheme.surfaceContainerLow
        case "outlined":
        case "text":     return "transparent"
        default:         return GreeterTheme.secondaryContainer // tonal
        }
    }

    // Label / state-layer "on" color per variant.
    function _contentColor() {
        if (!control.enabled) return Qt.alpha(GreeterTheme.onSurface, Tokens.state.disabledContentOpacity)
        switch (variant) {
        case "filled": return GreeterTheme.onPrimary
        case "tonal":  return GreeterTheme.onSecondaryContainer
        default:       return GreeterTheme.primary // elevated, outlined, text
        }
    }
    ...
    background: Item {
        ...
        Rectangle {
            ...
            border.color: control.variant === "outlined"
                    ? (control.enabled ? GreeterTheme.outline : Qt.alpha(GreeterTheme.onSurface, Tokens.state.disabledContainerOpacity))
                    : "transparent"
