...
import qs.src.features.notifications as Notifications
...
PanelWindow {
    ...
    function openPopout(name, data, source) {
        ...
        root.screen = getSourceScreen() || Quickshell.screens[0] || root.screen
    ...
    function closePopout() {
        ...
        if (root.currentName === "notification-center")
    ...
    Connections {
        ...
        function onNotificationCenterOpenChanged() {
            if (GlobalStates.notificationCenterOpen) {
                if (root.currentName !== "notification-center") {
                    root.openPopout("notification-center", null, null)
                }
            } else if (root.currentName === "notification-center") {
                root.closePopout()
            }
        }
    }
    ...
    function sourceGlobalPos() {
        ...
        return { x: (win ? win.x : 0) + posInWindow.x, y: (win ? win.y : 0) + posInWindow.y }
    }
    ...
    function getSourceScreen() {
        ...
        for (let i = 0; i < Quickshell.screens.length; i++) {
            ...
            if (globalPos.x >= s.x && globalPos.x < s.x + s.width &&
